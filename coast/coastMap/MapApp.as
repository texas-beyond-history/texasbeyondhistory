...
class MapApp extends mx.screens.Form{
	...
	function onLoad(){
		...
		navWin = mx.managers.PopUpManager.createPopUp(this, Window, false, {title:"Explore Map", contentPath:"navigationWin", closeButton:false});
		navWin.visible = false;
		navWin.move(-1000,-1000); //MAP REMOVE HACK : JUST IN CASE YOU WANT TO PUT THE MAP BACK :: simply change the x & y back to stage position
				
		// Creating the window for the legend.
		legendWin = mx.managers.PopUpManager.createPopUp(this, Window, false, {title:"Legend", contentPath:"lgndInitial", closeButton:false});
		legendWin.move(585,480);
